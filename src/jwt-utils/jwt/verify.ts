...
export async function verify(jwtString: string, secretOrPublicKey: string, options: VerifyOptions = {
  ...
  if (hasSignature && !secretOrPublicKey) {
    throw new JwtError(JwtErrorCode.INVALID_CREDENTIAL, 'secret or public key must be provided');
  }
  ...
  if (options.nonce) {
    if (payload.nonce !== options.nonce) {
      throw new JwtError(JwtErrorCode.INVALID_ARGUMENT, 'jwt nonce invalid. expected: ' + options.nonce);
    }
  }
