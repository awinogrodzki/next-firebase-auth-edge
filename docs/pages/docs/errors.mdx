...
| `NO_MATCHING_KID` | Handled internally to re-throw as `INVALID_ARGUMENT`. Usually indicates that certificate this token has been signed with has expired. This is **expected**. Google uses key rotation and refreshes certificates periodically. |
