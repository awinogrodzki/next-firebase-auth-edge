...
| serviceAccount   | `{ projectId: string; clientEmail: string; privateKey: string }` | Optional (required unless in [Google Cloud Run](https://cloud.google.com/run) environment or when `credential` is provided) | Firebase Service Account credentials.                                                                                                                          |
| credential       | `Credential`                                                     | Optional                                                                                   | A custom [Credential](/docs/usage/workload-identity-federation#the-credential-interface) implementation used to authorize requests to Google APIs and sign custom tokens remotely. Takes precedence over `serviceAccount`. Useful with [Workload Identity Federation](/docs/usage/workload-identity-federation). |
...
| serviceAccountId | `string`                                                         | Optional                                                                                   | The service account email used to sign custom tokens. Use in a [Google Cloud Run](https://cloud.google.com/run) environment or together with `credential` to skip the metadata server lookup.                                                        |
