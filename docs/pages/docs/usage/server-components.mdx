...
| serviceAccount      | Optional in authenticated [Google Cloud Run](https://cloud.google.com/run) environments or when `credential` is provided. Otherwise, **required**. This refers to the Firebase Service Account credentials.          |
...
| Name       | Description                                                                                                                                                                                                                                                                         |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| credential | **Optional** `Credential`. A custom [Credential](/docs/usage/workload-identity-federation#the-credential-interface) implementation used to authorize requests to Google APIs. Takes precedence over `serviceAccount`. Useful with [Workload Identity Federation](/docs/usage/workload-identity-federation). |
