...
export async function middleware(request: NextRequest) {
  return authMiddleware(request, {
    handleValidToken: async ({ decodedToken }, headers) => {
      const shouldRefreshCredentials = await makeSomeComputationsToDeduceIfUserCredentialsShouldBeUpdated(decodedToken.uid);

      if (shouldRefreshCredentials) {
        return refreshCredentials(request, commonOptions, ({headers}) =>
          NextResponse.next({
            request: {
              headers
            }
...
export async function POST(request: NextRequest) {
  ...
  return refreshNextResponseCookiesWithToken(token, request, response, commonOptions);
}
```
