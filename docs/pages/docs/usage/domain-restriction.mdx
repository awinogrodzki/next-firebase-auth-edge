...
3. If you are using either of `getCustomIdAndRefreshTokens`, `verifyIdToken`, `handleTokenRefresh`, `verifyAndRefreshExpiredIdToken` [advanced methods](/docs/usage/advanced-usage), make sure to pass `referer` option. `referer` should be value derived from incoming request's `Referer` header. You can use `getReferer` function imported from `next-firebase-auth-edge/lib/next/utils` to extract referer from `NextResponse`'s headers.
...
export async function POST(request: NextRequest) {
  ...
  await verifyIdToken(tokens.token, {
