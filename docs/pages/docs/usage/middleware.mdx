...
Authentication Middleware is mainly responsible for managing, validating and refreshing browser cookies. It's also a perfect place to implement custom routing behaviour based on user credentials.
...
**Please note** that you don't need to pass `headers` as in [middleware token verification caching](#middleware-token-verification-caching) when using `next-intl` middleware. `request` is already updated with modified headers by the time we call `i18nMiddleware`. `next-intl` will pass modified headers for us.
