...
| serviceAccount                       | Optional in authenticated [Google Cloud Run](https://cloud.google.com/run) environments or when `credential` is provided. Otherwise **required** |                                                                                                                                                                 | The Firebase project service account.                                                                                                                                                                                                                                                                                                                                                                             |
| credential                           | Optional                                                                                                        | `Credential`, defaults to `undefined`                                                                                                                           | A custom [Credential](/docs/usage/workload-identity-federation#the-credential-interface) implementation used to authorize requests to Google APIs and sign custom tokens remotely. Takes precedence over `serviceAccount`. Useful with [Workload Identity Federation](/docs/usage/workload-identity-federation) and other setups where service account keys cannot be used.                                       |
